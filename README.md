# Cart AI API

API para uma aplicação de carrinho inteligente com IA, estruturando regras de negócio, dados e integrações do backend.

![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=plastic&logo=typescript&logoColor=fff)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=plastic&logo=node.js)
![Prisma](https://img.shields.io/badge/Prisma-2D3748?style=plastic&logo=prisma)
![Jest](https://img.shields.io/badge/Jest-C21325?style=plastic&logo=jest&logoColor=fff)
![Zod](https://img.shields.io/badge/Zod-3E67B1?style=plastic&logo=zod&logoColor=fff)
![ESLint](https://img.shields.io/badge/ESLint-4B32C3?style=plastic&logo=eslint&logoColor=fff)

## Sobre

API para uma aplicação de carrinho inteligente com IA, estruturando regras de negócio, dados e integrações do backend.

## Funcionalidades

- API para aplicacao de carrinho inteligente com IA.
- Organizacao de regras de negocio e dados do backend.
- Endpoints para suportar fluxos de compra e recomendacao.
- Estrutura voltada a integracoes com frontend.

## Stack

- TypeScript
- Node.js
- Prisma
- Jest
- Zod
- ESLint

## Como executar

```bash
npm install
npm run start
```

## Scripts disponiveis

```bash
npm run build
npm run start
npm run test
npm run lint
npm run test:e2e
```

## Destaques tecnicos

- Backend TypeScript para suporte a funcionalidades com IA.
- Separacao entre regras de negocio e contratos de API.
- Projeto complementar ao Cart AI Web.

## Autor

<div align="center">
  <img src="https://github.com/marquesmaycon.png" width="100px" style="border-radius: 50%"/>
  <br/>
  <strong>Maycon Marques</strong>
  <br/>
  <br/>

  [![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=data:image/svg+xml;base64,)](https://www.linkedin.com/in/mayconhenrique/)
  [![GitHub](https://img.shields.io/badge/GitHub-100000?style=flat-square&logo=github&logoColor=white)](https://github.com/marquesmaycon)
  [![Email](https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:mayconmarquesh@gmail.com)
  [![WhatsApp](https://img.shields.io/badge/WhatsApp-25D366?style=flat-square&logo=whatsapp&logoColor=white)](https://wa.me/5544991173753)
</div>
